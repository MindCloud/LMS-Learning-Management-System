...
const strengthLabel = (pw) => {
  ...
  const passed = [c.len, c.upper, c.num].filter(Boolean).length;
  return ["Weak", "Fair", "Good"][Math.max(0, passed - 1)];
};

function AddTeacherForm({ onClose, onTeacherAdded }) {
  ...
  const validate = async () => {
    ...
    if (!e.username) {
      ...
      if (!ur.empty) e.username = "This username is already in use.";
    }
    if (!e.email) {
      const eq = query(
        collection(db, "teachers"),
      ...
      if (!er.empty) e.email = "This email is already registered.";
    }
        ...
        <div className="sticky top-0 z-10 flex items-center justify-between border-b bg-gradient-to-r from-blue-50 to-white px-6 py-4">
          <div>
            ...
            <p className="text-xs text-blue-700/70">
              Create an account and profile for a teacher
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="sticky bottom-0 z-10 border-t bg-gradient-to-r from-white to-blue-50 px-6 py-4">
          <div className="flex flex-col sm:flex-row items-center justify-end gap-3">
            <button
              type="button"
              onClick={onClose}
              className="w-full sm:w-auto rounded-xl border border-blue-200 px-5 py-3 text-sm font-medium text-blue-800 transition hover:bg-blue-50 active:scale-95"
            >
              Cancel
            </button>
            <button
