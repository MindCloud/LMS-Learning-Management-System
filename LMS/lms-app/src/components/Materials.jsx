// src/components/Materials.jsx - Updated with modern visual aesthetics and framer-motion transitions
...
function Materials() {
  ...
  const [searchFilter, setSearchFilter] = useState(""); // Material search
  ...
  // Format creation timestamp relatively
  const formatMaterialDate = (createdAt) => {
  ...
  // Grade badge styling colors helper
  ...
  // Filter materials for lists feed
  const filteredMaterials = materials.filter((material) => {
    const matchesSearch =
      material.title.toLowerCase().includes(searchFilter.toLowerCase()) ||
      material.link.toLowerCase().includes(searchFilter.toLowerCase());

    const matchesGrade =
      gradeFilter === "all" ||
      (material.grades && Array.isArray(material.grades) && material.grades.includes(gradeFilter)) ||
      (material.grades && material.grades.length === 0 && gradeFilter === "All Grades");
      ...
      <div className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8">
        ...
        <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-6 border-b border-slate-200/60 mb-8">
          <div className="flex items-center gap-3">
            <div className="relative flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-tr from-purple-600 to-indigo-600 text-white shadow-md shadow-indigo-500/15">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">
                Share reference files, video lessons, and resources with your students
              </p>
            </div>
          </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g., Algebra: Linear Equations – Notes"
                    className="w-full rounded-xl border border-slate-200 bg-white/80 px-4 py-2.5 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-500/25 focus:border-purple-500 transition-all disabled:opacity-70"
                    disabled={adding}
                  />
                </div>
                ...
                <div className="relative">
                  ...
                  <button
                    ...
                    disabled={adding}
                    className="flex w-full items-center justify-between rounded-xl border border-slate-200 bg-white/85 px-4 py-3 text-sm text-slate-700 hover:bg-slate-50 transition focus:outline-none focus:ring-2 focus:ring-purple-500/25 disabled:opacity-70"
                  >
                    <span className="text-slate-500 font-medium">
                      {selectedGrades.length === 0
                        ? "Select targeted grades"
                  ...
                  {/* Active Selected Tags representation */}
                  {selectedGrades.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mt-2.5">
                      {selectedGrades.map((grade) => (
                        <span
                          ...
                          className="inline-flex items-center gap-1 cursor-pointer rounded-lg bg-purple-50 border border-purple-100/80 pl-2.5 pr-1.5 py-1 text-xs font-semibold text-purple-700 hover:bg-red-50 hover:text-red-700 hover:border-red-100 transition-colors"
                ...
                <button
                  onClick={addMaterial}
                  disabled={adding}
                  className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 px-4 py-3 text-sm font-semibold text-white shadow-sm hover:shadow-md hover:from-purple-700 hover:to-indigo-700 active:scale-98 transition-all disabled:opacity-75 disabled:cursor-not-allowed cursor-pointer mt-2"
                >
                  {adding ? (
            ...
            <div className="relative overflow-hidden rounded-2xl border border-slate-200/60 bg-white/70 backdrop-blur-md p-6 shadow-sm">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-xl border border-slate-100 bg-slate-50/50 p-4">
                  <span className="block text-xs font-semibold text-slate-400 uppercase tracking-wider">Shared Materials</span>
                  <span className="text-2xl font-bold text-slate-800">{materials.length}</span>
                </div>
                <div className="rounded-xl border border-slate-100 bg-slate-50/50 p-4">
                  <span className="block text-xs font-semibold text-slate-400 uppercase tracking-wider">Filtered Feed</span>
                  <span className="text-2xl font-bold text-purple-600">{filteredMaterials.length}</span>
                </div>
              </div>
            </div>
          </div>

          {/* Shared Materials List Feed: Span 7 */}
          <div className="lg:col-span-7 flex flex-col space-y-4">
            {/* Search and Grade filters row */}
            <div className="flex flex-col sm:flex-row gap-3 rounded-2xl border border-slate-200/60 bg-white/70 backdrop-blur-md p-4 shadow-sm">
              <div className="relative flex-1">
                <input
                  ...
                  className="w-full rounded-xl border border-slate-200 bg-white/80 px-4 py-2.5 pl-10 text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all"
                />
              ...
              <div className="relative sm:w-48">
                <select
                  ...
                  className="w-full rounded-xl border border-slate-200 bg-white/80 px-4 py-2.5 text-sm text-slate-700 appearance-none focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all cursor-pointer font-semibold"
                >
                ...
                <div className="flex flex-col items-center justify-center gap-4 rounded-2xl border border-dashed border-slate-200 bg-white/40 backdrop-blur-sm p-12 text-center h-full">
                  ...
                  <div>
                    <h3 className="font-bold text-slate-800 text-lg">
                      {searchFilter || gradeFilter !== "all" ? "No resources found" : "No materials shared yet"}
                    </h3>
                    <p className="text-xs text-slate-500 mt-1 max-w-sm">
                      {searchFilter || gradeFilter !== "all"
                        ? "Try adjusting your search terms or grade filter settings."
                        : "Post your first lecture slides or notes using the composer on the left to share with students."}
                    </p>
                  </div>
                </div>
              ) : (
                  ...
                  <AnimatePresence mode="popLayout">
                    {filteredMaterials.map((m) => (
                      <motion.li
                        ...
                        className="group relative flex flex-col rounded-2xl border border-slate-200/60 bg-white/80 backdrop-blur-md p-5 shadow-2xs hover:shadow-xs hover:border-purple-200/70 transition-all duration-300 overflow-hidden"
                      >
                        {/* Left visual accent indicator bar */}
                        <div className="absolute left-0 top-0 bottom-0 w-[4px] bg-gradient-to-b from-purple-500 to-indigo-500 rounded-r-lg group-hover:from-purple-600 group-hover:to-indigo-600 transition-colors" />
                        ...
                        <div className="flex items-start justify-between gap-4">
                          ...
                          >
                            <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-purple-50 text-purple-600 shrink-0 mt-0.5 group-hover:scale-105 transition-transform duration-200">
                              <LinkIcon className="h-4.5 w-4.5" />
                            </div>
                            <div className="flex-1 min-w-0">
                              <h4 className="text-slate-800 text-sm font-bold leading-relaxed truncate group-hover:text-purple-600 transition-colors">
                                {m.title}
                              </h4>
                              <p className="text-xs text-slate-400 truncate max-w-md mt-0.5 font-medium">
                                {m.link}
                              </p>
                            ...
                            <ExternalLink className="h-4 w-4 text-slate-400 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity mt-1 self-start" />
                          </a>
                        ...
                        <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-400 font-medium">
                          <div className="flex items-center gap-1.5" title={m.createdAt?.toDate ? m.createdAt.toDate().toLocaleString() : "Just now"}>
                            <Clock className="h-3.5 w-3.5 text-slate-300" />
                            <span>Shared {formatMaterialDate(m.createdAt)}</span>
                          </div>
                        </div>
                      </motion.li>
                    ))}
