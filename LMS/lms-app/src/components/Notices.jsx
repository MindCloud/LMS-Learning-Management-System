// src/components/Notices.jsx - Styled with Gradient Orange and Yellow Theme
...
function Notices() {
  ...
  // Grade color tags generator (orange-yellow theme harmonized)
  const getGradeTagStyles = (grade) => {
    const normalized = grade.toLowerCase();
    if (normalized.includes("o/l")) {
      return "bg-amber-50 dark:bg-amber-950/25 text-amber-700 dark:text-amber-400 border-amber-200/65 dark:border-amber-900/30";
    }
    if (normalized.includes("a/l")) {
      return "bg-orange-50 dark:bg-orange-950/25 text-orange-700 dark:text-orange-400 border-orange-200/65 dark:border-orange-900/30";
    }
    if (normalized.includes("10") || normalized.includes("11")) {
      return "bg-yellow-50 dark:bg-yellow-950/25 text-yellow-750 dark:text-yellow-400 border-yellow-250/65 dark:border-yellow-900/30";
    }
    return "bg-rose-50 dark:bg-rose-950/15 text-rose-700 dark:text-rose-450 border-rose-100/50 dark:border-rose-950/30";
  };
      ...
      <div className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8">
        ...
        <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-6 border-b border-slate-200/60 dark:border-slate-800/60 mb-8">
          <div className="flex items-center gap-3">
            <div className="relative flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-tr from-orange-500 via-amber-500 to-yellow-500 text-white shadow-md shadow-orange-500/20">
                ...
                <div>
                  ...
                  <div className="relative">
                    <textarea
                      ...
                      rows={5}
                      disabled={submitting}
                      className="w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-950 p-4 text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all resize-none disabled:opacity-70"
                    />
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-2.5">
                      {selectedGrades.map((grade) => (
                        <span
                          ...
                          className="inline-flex items-center gap-1 cursor-pointer rounded-lg bg-orange-50 dark:bg-orange-950/20 border border-orange-100/80 dark:border-orange-900/30 pl-2.5 pr-1.5 py-1 text-xs font-semibold text-orange-700 dark:text-orange-400 hover:bg-red-50 dark:hover:bg-red-950/40 hover:text-red-700 hover:border-red-100 transition-colors"
                              ...
                              return (
                                <button
                                  ...
                                  className={`flex items-center gap-2 rounded-lg px-2.5 py-2 text-left text-xs font-semibold transition-all ${isSelected
                                    ? "bg-orange-50 dark:bg-orange-950/35 text-orange-750 dark:text-orange-350 border border-orange-100/50 dark:border-orange-900/30"
            ...
            <div className="relative overflow-hidden rounded-2xl border border-slate-200/60 dark:border-slate-800/60 bg-white/70 dark:bg-slate-900/60 backdrop-blur-md p-6 shadow-sm">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/40 p-4">
                  <span className="block text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Active Notices</span>
                  <span className="text-2xl font-bold text-slate-800 dark:text-slate-200">{notices.length}</span>
                </div>
                <div className="rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/40 p-4">
                  ...
                  <span className="text-2xl font-bold text-orange-600 dark:text-orange-455">{filteredNotices.length}</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 rounded-2xl border border-slate-200/60 dark:border-slate-800/60 bg-white/70 dark:bg-slate-900/60 backdrop-blur-md p-4 shadow-sm">
              <div className="relative flex-1">
                <input
                  ...
                  className="w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-950/50 px-4 py-2.5 pl-10 text-sm text-slate-700 dark:text-slate-250 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all"
                />
              ...
              <div className="relative sm:w-48">
                <select
                  ...
                  className="w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-950/50 px-4 py-2.5 text-sm text-slate-700 dark:text-slate-200 appearance-none focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all cursor-pointer font-semibold"
                >
                  ...
                  <AnimatePresence mode="popLayout">
                    {filteredNotices.map((notice) => (
                      <motion.li
                        ...
                        className="group relative flex flex-col rounded-2xl border border-slate-200/60 dark:border-slate-800/60 bg-white/80 dark:bg-slate-900/50 backdrop-blur-md p-5 shadow-2xs hover:shadow-xs hover:border-orange-200/60 transition-all duration-300 overflow-hidden"
                      >
                        {/* Left visual indicator accent border */}
                        <div className="absolute left-0 top-0 bottom-0 w-[4px] bg-gradient-to-b from-orange-500 to-yellow-450 rounded-r-lg group-hover:from-orange-600 group-hover:to-yellow-550 transition-colors" />

                        <div className="flex items-start justify-between gap-4">
                          <div className="flex items-start gap-3 flex-1 min-w-0">
                            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-orange-50 dark:bg-orange-950/20 text-orange-600 dark:text-orange-400 shrink-0 mt-0.5 animate-pulse">
